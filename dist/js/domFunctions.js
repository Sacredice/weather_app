...
export const displayApiError = function(statusCode) {
...
export const updateDisplay = (weatherJson, locationObj, forecastJson) => {
    ...
    // six day forecast
    displayFiveDayForecast(forecastJson);
    setFocusOnSearch();
...
const createCurrentConditionsDivs = (weatherObj, unit) => {
    ...
    // array sırası önemli icon ve temp sonrası sıralama burdaki sıraya göre olur.
...
const translateIconToFontAwesome = (icon) => {
    ...
    switch (firstTwoChars) {
        ...
        case "10":
            if (lastChar === "d") {
                i.classList.add("fas", "fa-sun-rain");
            } else {
                i.classList.add("fas", "fa-moon-rain")
            }
            break;
        case "11":
...
const displayFiveDayForecast = (forecastJson) => {
    for (let i = 7; i <= 39; i += 8) {
        const dfArray = createDailyForecastDivs(forecastJson.list[i]);
...
const createDailyForecastDivs = (dayWeather) => {
    ...
    const dayLow = createElem("p", "dayLow", `${Math.round(Number(dayWeather.main.temp_max))}°`)
...
const createDailyForecastIcon = (icon, desc) => {
    ...
    if (window.innerHeight < 768 || window.innerWidth < 1025) {
